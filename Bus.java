...
public class Bus implements Comparable<Bus> {
  ...
  public int compareTo(Bus other) {
    int time = this.arrivalTime.compareTo(other.getArrivalTime()); // comparison var for time
    if (time != 0)
      return time;

    int name = this.getName().compareTo(other.getName());// comparison var for name
    if (name != 0)
      return name;

    int direction = this.route.getDirection().compareTo(other.route.getDirection());// comparison
                                                                                    // var for dir
    if (direction != 0)
      return direction;
